...
Academic_subject = input("Enter 5 subjects separated by comma: ").split(",")
# collects the 5 relevants subject 
academic_qualiafication = [input(f"Enter grade for {subject}: ") for subject in Academic_subject] # collects grades for the entered subject
accepeted_grade = ("A" in academic_qualiafication == 5) or ("B" in academic_qualiafication == 5) # checks the length of the A/Bs

eligibility = (nationality == "nigerian") and (employment_status == "undergraduate") and (Scholar_enrolled == "no") and (accepeted_grade == True) #confirms eligiblity

print(f"Candidate:{name}\nCitizenship:{nationality}\nEmployment Status:{employment_status}\nEnrollment: {Scholar_enrolled}\nEligible: {eligibility}")
print(accepeted_grade)